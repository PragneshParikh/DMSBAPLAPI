...
namespace DMS_BAPL_Api.Controllers
{
    ...
    public class JobCardController : ControllerBase
    {
        ...
        [HttpGet("GetAllInspectedChassis/{dealerCode}/{jobTypeId}")]
        [ProducesResponseType(typeof(PagedResponse<object>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        public async Task<IActionResult> GetServiceHistory(string chassisNo, int? jobCardId)
        {
            ...
            {
                _logger.LogError(ex, "Error in GetServiceHistory");
                return StatusCode(500, "An error occurred while fetching service history.");
            }

        }

        [HttpGet("GetCIRJobCardDetails/{id}")]
        [ProducesResponseType(typeof(PagedResponse<object>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> GetCIRJobCardDetails(int id)
        {
            try
            {
                string userId = GetUserInfoFromToken.GetUserIdFromToken(HttpContext);
                if (string.IsNullOrEmpty(userId))
                    return Unauthorized("User not authorized");
                var result = await _jobCardRepo.GetCIRJobCardDetails(id);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("GetJobCardListRepairBill")]
        [ProducesResponseType(typeof(PagedResponse<object>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> GetJobCardListRepairBill([FromQuery] JobCardSearchVM search)
        {
            try
            {
                string userId = GetUserInfoFromToken.GetUserIdFromToken(HttpContext);
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> GetIssueTypebasedJobDetails( [FromQuery] string? dealerCode, [FromQuery] int? jobNo,
                            [FromQuery] string? serviceloc, [FromQuery] DateTime? fromDate,[FromQuery] DateTime? toDate)
        {
            try
            {
