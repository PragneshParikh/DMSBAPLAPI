...
namespace DMS_BAPL_Api.Controllers
{
    ...
    public class VehicleOpeningStockController : ControllerBase
    {
        ...
        [HttpGet("GetVehicleSaleDetailsByModel")]
        [ProducesResponseType(typeof(List<VehicleOpeningDetailsVM>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> VehicleOpeningStockDetails(string? modelName)
        {
            try
            {
                var result = await _vehicleOpeningStock.GetVehicleSaleDetailsByModelAsync(modelName);
                return Ok(result);
            }
            catch (Exception ex)
            {
